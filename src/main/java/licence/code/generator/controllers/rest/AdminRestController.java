...
public class AdminRestController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "Unable to find User for given Id"),
            @ApiResponse(responseCode = "409", description = "User is already blocked"),
    })
    @PreAuthorize("hasRole('ADMIN')")
    @PostMapping(value = {ADMIN_BLOCK_PATH})
    public ResponseEntity<?> blockUser(@Valid @RequestBody IdRequestDto idRequestDto) {
        User requester = getRequester();
        LOGGER.info("Admin with id: {} attempts to block User with id: {}", requester.getId(), idRequestDto.id());
    ...
    public ResponseEntity<?> unblockUser(@Valid @RequestBody IdRequestDto idRequestDto) {
        User requester = getRequester();
        LOGGER.info("Admin with id: {} attempts to unblock User with id: {}", requester.getId(), idRequestDto.id());
